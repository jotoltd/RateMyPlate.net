  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Discover Chefs</h1>
          <p className="text-sm text-gray-500">Find talented food creators to follow</p>
        </div>
      </div>
          ...
          {chefs.map((chef) => {
            ...
            return (
              <Link
                ...
                className="flex items-center gap-4 p-4 bg-white dark:bg-gray-900 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-xl hover:border-orange-200 dark:hover:border-orange-700 transition-all group"
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-gray-900 dark:text-white group-hover:text-orange-500 transition-colors truncate">
                    @{chef.username}
                    {isOwn && <span className="ml-2 text-[10px] font-bold bg-orange-100 text-orange-500 px-1.5 py-0.5 rounded-full">You</span>}
                  </p>
                  {chef.bio && (
                    <p className="text-xs text-gray-500 line-clamp-1 mt-0.5">{chef.bio}</p>
                  )}
