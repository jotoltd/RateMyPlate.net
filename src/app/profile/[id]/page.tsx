...
export default async function ProfilePage({
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      {/* Tab switcher */}
      <div className="flex gap-1 bg-white/[0.04] rounded-2xl p-1 mb-6 w-fit">
        {[
      ...
      {rows.map((r) => {
        ...
        return (
          <Link
            key={r.id}
            href={`/plate/${plate.id}`}
            className="flex items-center gap-4 p-4 bg-white/[0.03] border border-white/5 hover:border-orange-500/20 rounded-2xl transition-all"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-white/30">by @{plate.profiles?.username ?? "chef"} · {formatDate(r.created_at)}</p>
              {r.comment && <p className="text-xs text-white/40 line-clamp-1 mt-0.5 italic">"{r.comment}"</p>}
            </div>
