...
do $$ begin
  if not exists (select 1 from pg_policies where tablename = 'collections' and policyname = 'Collections viewable by owner') then
    create policy "Collections viewable by owner" on public.collections for select using (auth.uid() = user_id);
  end if;
  ...
  if not exists (select 1 from pg_policies where tablename = 'collection_plates' and policyname = 'Collection plates viewable by collection owner') then
    create policy "Collection plates viewable by collection owner" on public.collection_plates for select
      using (exists (select 1 from public.collections c where c.id = collection_id and c.user_id = auth.uid()));
  end if;
  if not exists (select 1 from pg_policies where tablename = 'collection_plates' and policyname = 'Users can add plates to own collections') then
    create policy "Users can add plates to own collections" on public.collection_plates for insert
      with check (exists (select 1 from public.collections c where c.id = collection_id and c.user_id = auth.uid()));
  end if;
  if not exists (select 1 from pg_policies where tablename = 'collection_plates' and policyname = 'Users can remove plates from own collections') then
    create policy "Users can remove plates from own collections" on public.collection_plates for delete
      using (exists (select 1 from public.collections c where c.id = collection_id and c.user_id = auth.uid()));
  end if;
end $$;
